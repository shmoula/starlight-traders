...
| :---- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----- |
| E2-2f | **Contract rewards are anchored to a stale price.** `reward` uses `getPrice(seed, **offer day**, destination, commodity)`, but the player buys at the destination a day or more later. High-volatility luxury lets that anchor go stale by hundreds of cr/unit. This is what keeps the 2-jump re-qualification line alive after E2-2d: buy at destination B, bounce B→C→B so `boughtHere` resets, deliver as hauled. Measured over 11,944 contracts it beats the best honest route outright in 16.1% (median +127cr, top +4,297cr), 2.7% per day, and 0.9% even against contract-free arbitrage with the same capital. Dominated on the mean (−4.5% of reward) and capital-hungry, so not urgent — but the mean was what made it look closed. | Re-anchor the reward to the destination's price on the **delivery** day, or price the premium off a day-independent base. Coordinate with **E2-1**: market depth changes the volatility this feeds on. | S      |
| E2-2g | **Hauled units go to the earliest-accepted contract, with no UI cue.** Settlement drains the shared hauled pool in `activeMissions` order, so accepting a whale before a minnow is strictly better when both want the same commodity — a real decision the player cannot see. Not a bug; an invisible rule.                                                                                                                                                                                                                                                                                                                                                                                                                                   | Either surface the settlement order on the active cards, or make it deterministic on something the player can reason about (highest reward first).                                                     | S      |
| E2-2h | **A dock-side action can strand you with no loss detected.** `checkLoss` has one caller, inside `arrive`, and `jump` returns unchanged when fuel is short — so spending down at a station (a bond escrow, a repair) can leave every jump unaffordable with `status` still `playing`. The only exit is Retire, which pays a survival bonus, so stranding yourself this way scores _better_ than being stranded. Pre-existing class; E2-2a adds an entry point that is irreversible by design.                                                                                                                                                                                                                                                  | Run `checkLoss` after dock actions, or guard actions that would drop credits below the cheapest escape.                                                                                                | S      |
| E2-2i | **`deliver()` does not update `peakNetWorth`.** `arrive()` wraps settlement in `trackPeak`; `deliver()` does not, so a dockside delivery raises net worth without moving the run's high-water mark and the debrief under-reports it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Wrap `deliver`'s return in `trackPeak` and add a regression test.                                                                                                                                      | XS     |
| E2-2j | **`biggestPayday` and the `bigTrade` day-highlight now include the player's own returned bond.** "Best haul" on a 5,000cr contract reads 5,500cr, and an 850cr contract can cross `BIG_TRADE_CR` on the refund alone. Spec-sanctioned (decision 1 uses the actual inflow) and arguably right, since the stat then matches the log line — but it inflates the share strip.                                                                                                                                                                                                                                                                                                                                                                     | Decide whether the debrief stat should read gross inflow or earned reward, before further share-strip work.                                                                                            | XS     |
